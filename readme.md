...
* [注销登录 /api/auth/logout](Document/post/auth/logout.md)
...
1000 | 批量数据校验错误 | 系统针对批量数据校验统一给出 `1000` 状态码，参照 3.4 响应字段详情

### 3.4 数据格式校验响应

状态码 1000 响应表示数据校验错误，由于可以同时处理多个字段校验，响应格式为字段组成的对象。当该字段校验不通过时，便会出现该字段的校验结果对象。
