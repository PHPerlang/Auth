...
class CreateAuthMembersTable extends Migration
{
    public function up()
    {
        Schema::create('auth_members', function (Blueprint $table) {
            ...
            $table->string('member_name')->nullable()->comment('系统用户姓名');
            $table->tinyInteger('member_status')->default(1)->comment('系统用户状态, 0 为禁用状态 1 为激活状态');
            $table->string('wechat_open_id')->nullable()->comment('微信OPENID');
            $table->string('register_channel')->comment('注册通道');
            $table->string('register_source')->nullable()->comment('注册来源');
            ...
            $table->timestamp('last_login')->nullable()->comment('系统用户创建时间');
